...
public class PmsProductController {
	...
	public CommonResult create(@RequestBody PmsProductParam productParam, BindingResult bindingResult) {
		// 增加一些默认字段
		PmsProductParam newProductParam = productParam;
		if (newProductParam.getDeleteStatus() == null) {
			newProductParam.setDeleteStatus(0);
		}
		if (newProductParam.getVerifyStatus() == null) {
            newProductParam.setVerifyStatus(0);
        }
		if (newProductParam.getSale() == null) {
		    newProductParam.setSale(0);
        }
		if (newProductParam.getFeightTemplateId() == null) {
		    newProductParam.setFeightTemplateId(new Long(0));
        }
		if (newProductParam.getLowStock() == null) {
		    newProductParam.setLowStock(0);
        }
		int count = productService.create(newProductParam);
		if (count > 0) {
